...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "total_buildings", type=int, help="How many buildings to create"
        )
        parser.add_argument(
            "--delete",
            action="store_true",
    ...
    def handle(self, *args, **options):
        if options["delete"]:
            ...
            Building.objects.filter(
                general_id__in=list(
                    Building.objects.values_list("pk", flat=True)[
                        : int(options["total_buildings"])
                    ]
                )
            ).delete()
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully deleted %s building(s)."
                    % (int(buildings_count))
                )
            )
        elif options["create"]:
            for building_id in trange(int(options["total_buildings"])):
                ...
                risk_category = random.choice(
                    ["NA", "U1", "U2", "U3", "U4", "RS1", "RS2", "RS3", "RS4"]
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully created %s building(s)." % (building_id + 1)
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR(
                    "Missing required argument: --create or --delete"
                )
            )
            self.stdout.write(
                self.style.SUCCESS(
                    "Try: python manage.py buildings %s --create"
                    % (int(options["total_buildings"]))
                )
            )
